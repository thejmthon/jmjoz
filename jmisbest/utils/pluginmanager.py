...
from jmisbest import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"jmisbest/plugins/{shortname}.py")
        checkplugins(path)
        name = f"jmisbest.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"jmisbest.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    jmisbest.remove_event_handler(i)
    ...
    try:
        name = f"jmisbest.plugins.{shortname}"
        for i in reversed(range(len(jmisbest._event_builders))):
            ev, cb = jmisbest._event_builders[i]
            if cb.__module__ == name:
                del jmisbest._event_builders[i]
