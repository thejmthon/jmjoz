...
from . import CMD_HELP, CMD_LIST, SUDO_LIST, edit_delete, edit_or_reply, jmisbest
...
def plug_checker(plugin):
    plug_path = f"./jmisbest/plugins/{plugin}.py"
    if not os.path.exists(plug_path):
        plug_path = f"./xtraplugins/{plugin}.py"
    return plug_path


@jmisbest.ar_cmd(pattern="تنصيب$")
...
@jmisbest.ar_cmd(pattern="الغاء تنصيب ([\s\S]*)")
