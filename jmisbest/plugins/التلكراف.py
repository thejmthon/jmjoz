...
from . import BOTLOG, BOTLOG_CHATID, jmisbest
...
@jmisbest.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    jmisbestevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if not event.reply_to_msg_id:
        return await jmisbestevent.edit(
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await jmisbestevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await jmisbestevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
        ...
        ms = (end - start).seconds
        jmisbest = f"https://graph.org/{response['path']}"
        await jmisbestevent.edit(
            f"**الرابط**:  [اضغط هنا]({jmisbest})\
