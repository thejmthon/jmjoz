...
async def delpack(jmthonevent, conv, args, packname):
    try:
        ...
        await jmisbest(unblock("stickers"))
        await conv.send_message("/delpack")
...
@jmisbest.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@jmisbest.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@jmisbest.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@jmisbest.ar_cmd(pattern="معلومات_ألملصق$")
