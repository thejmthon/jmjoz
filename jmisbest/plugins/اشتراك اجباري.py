...
async def participant_check(channel, user_id):
    try:
        await jmisbest(GetParticipantRequest(channel, int(user_id)))
        return True
...
@jmisbest.ar_cmd(pattern="اجباري ?(.*)")
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, jmisbest.uid):
            return await event.reply(
...
@jmisbest.ar_cmd(pattern="تعطيل الاجباري")
