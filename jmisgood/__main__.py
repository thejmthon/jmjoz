...
import jmisgood
from jmisgood import BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
try:
    ...
    jmisgood.loop.run_until_complete(setup_bot())
...
try:
    ...
    jmisgood.loop.create_task(saves())
...
try:
    ...
    jmisgood.loop.run_until_complete(mybot())
...
async def startup_process():
    if jmisgood.uid in blacklisted_users:
...
jmisgood.loop.run_until_complete(startup_process())

if len(sys.argv) in {1, 3, 4}:
    with contextlib.suppress(ConnectionError):
        jmisgood.run_until_disconnected()
else:
    jmisgood.disconnect()
