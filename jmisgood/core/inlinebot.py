...
async def inline_handler(event):
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**jmisgood"):
            ...
            builder = event.builder
            troll = os.path.join("./jmisgood", "troll.txt")
            try:
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./jmisgood", "secrets.txt")
            try:
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./jmisgood", "hide.txt")
            try:
...
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
