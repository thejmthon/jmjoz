...
from jmisgood import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"jmisgood/plugins/{shortname}.py")
        checkplugins(path)
        name = f"jmisgood.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"jmisgood.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    jmisgood.remove_event_handler(i)
    ...
    try:
        name = f"jmisgood.plugins.{shortname}"
        for i in reversed(range(len(jmisgood._event_builders))):
            ev, cb = jmisgood._event_builders[i]
            if cb.__module__ == name:
                del jmisgood._event_builders[i]
