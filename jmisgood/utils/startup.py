...
from jmisgood import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
from razan.CMD.utils import *

from ..Config import Config
from ..core.logger import logging
from ..core.session import jmisgood
...
async def setup_bot():
    ...
    """
    try:
        await jmisgood.connect()
        config = await jmisgood(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == jmisgood.session.server_address:
                if jmisgood.session.dc_id != option.id:
                    LOGS.warning(
                        f"اصلاح الداتا {jmisgood.session.dc_id}" f" الى {option.id}"
                    )
                jmisgood.session.set_dc(option.id, option.ip_address, option.port)
                jmisgood.session.save()
                break
        bot_details = await jmisgood.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await jmisgood.start(bot_token=Config.TG_BOT_USERNAME)
        jmisgood.me = await jmisgood.get_me()
        jmisgood.uid = jmisgood.tgbot.uid = utils.get_peer_id(jmisgood.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(jmisgood.me)
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await jmisgood(UnblockRequest("@R0R77"))
        await jmisgood(UnblockRequest("@jmthon_bot"))
        await jmisgood(
        ...
        await jmisgood.edit_folder("@jmthon_bot", folder=1)  # عمل ارشيف للبوت
        channel_usernames = ["jmthon", "RR7PP", "thejmthon"]
        for channel_username in channel_usernames:
            try:
                channel = await jmisgood.get_entity(channel_username)
                await jmisgood(JoinChannelRequest(channel=channel))
            except Exception as e:
                LOGS.error(f"{e}")
...
async def mybot():
    jmisgood_USER = jmisgood.me.first_name
    The_razan = jmisgood.uid
    rz_ment = f"[{jmisgood_USER}](tg://user?id={The_razan})"
    f"ـ {rz_ment}"
    f"⪼ هذا هو بوت خاص بـ {rz_ment} يمكنك التواصل معه هنا"
    starkbot = await jmisgood.tgbot.get_me()
    perf = "[ جمثون ]"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await jmisgood.send_message("@jmthon_bot", "/start")
            await asyncio.sleep(1)
            await jmisgood.send_message(
                "@jmthon_bot",
                "تم بنجاح تشغيل سورس جمثون عزيزي المستخدم هذا البوت سيتم تشغيله قريبا بعد اكماله",
            )
            await asyncio.sleep(1)
            await jmisgood.send_message("@BotFather", "/setinline")
            await asyncio.sleep(1)
            await jmisgood.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await jmisgood.send_message("@BotFather", perf)
...
async def startupmessage():
    if not gvarstatus("DEPLOY"):
        try:
            if BOTLOG:
                await jmisgood.tgbot.send_file(
                    BOTLOG_CHATID,
        ...
        try:
            if BOTLOG:
                await jmisgood.tgbot.send_message(
                    BOTLOG_CHATID,
    ...
    try:
        if msg_details:
            await jmisgood.check_testcases()
            message = await jmisgood.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**الان السورس شغال طبيعي.**"
            await jmisgood.edit_message(msg_details[0], msg_details[1], text)
...
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"initft_(\\d+)")))
async def deploy(e):
...
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"initbk_(\\d+)")))
async def ineiq(e):
    CURRENT = int(e.data_match.group(1))
    if CURRENT == 1:
        return await e.edit(
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"jmisgood/{folder}/*.py"
        plugin_path = f"jmisgood/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await jmisgood.get_entity(BOTLOG_CHATID)
        ...
        photobt = await jmisgood.upload_file(file="razan/pic/Jmthonp.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت جمثون", jmisgood, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await jmisgood.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
        ...
        photobt = await jmisgood.upload_file(file="razan/pic/Jmthonp.jpg")
        _, groupid = await create_supergroup(
            "مجموعة التخزين", jmisgood, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await jmisgood.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await jmisgood.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="jmisgood", extfolder=cfolder)
