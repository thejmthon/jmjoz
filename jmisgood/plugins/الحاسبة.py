...
@jmisgood.ar_cmd(pattern="الحاسبة(?:\s|$)([\s\S]*)")
async def icalc(e):
    if e.client._bot:
        return await e.reply("**الحاسبة العلمية لسورس جمثون\n @jmisgood**", buttons=lst)
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @jmisgood  ~ @RR7PP
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @jmisgood  ~ @RR7PP
@jmisgood.tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
