...
@jmisgood.ar_cmd(pattern="اوامر التحكم")
async def hi(event):
    await edit_or_reply(
        event,
...
@jmisgood.ar_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@jmisgood.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@jmisgood.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
@jmisgood.ar_cmd(pattern="المتحكمين$")
async def _(event):
...
jmisgood.loop.create_task(_init())
