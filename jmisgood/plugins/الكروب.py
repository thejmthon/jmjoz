...
@jmisgood.ar_cmd(pattern="المشرفين(?: |$)(.*)")
async def _(event):
...
@jmisgood.ar_cmd(pattern="البوتات(?: |$)(.*)")
async def _(event):
...
@jmisgood.ar_cmd(pattern="الاعضاء(?: |$)(.*)")
async def get_users(show):
    ...
    jmisgoodevent = await edit_or_reply(show, "**⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(jmisgoodevent, mentions)


@jmisgood.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    jmisgoodevent = await edit_or_reply(
        event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**"
    )
    chat = await get_chatinfo(event, jmisgoodevent)
    caption = await fetch_info(chat, event)
    try:
        await jmisgoodevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**⌔∮ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await jmisgoodevent.edit("**⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, jmisgoodevent):
    ...
    try:
        ...
        try:
            ...
            await jmisgoodevent.edit("**⌔∮ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
        except ChannelPrivateError:
            await jmisgoodevent.edit(
                "**⌔∮ هذه مجموعة أو قناة خاصة أو لقد تمّ حظري منه ⛞**"
            )
            return None
        except ChannelPublicGroupNaError:
            await jmisgoodevent.edit("**⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await jmisgoodevent.edit(str(err))
            return None
    return chat_info
