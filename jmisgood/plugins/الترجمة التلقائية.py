...
@jmisgood.ar_cmd(pattern="تفعيل الترجمة")
...
@jmisgood.ar_cmd(pattern="تعطيل الترجمة")
...
@jmisgood.on(events.NewMessage(outgoing=True))
    ...
    if gvarstatus("translateen"):
        ...
        try:
            ...
            if translation.text != event.message.message:
                await jmisgood.edit_message(event.message, translation.text)
        except:
            pass
