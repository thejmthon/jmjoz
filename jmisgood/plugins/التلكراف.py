...
from . import BOTLOG, BOTLOG_CHATID, jmisgood
...
@jmisgood.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    jmisgoodevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if not event.reply_to_msg_id:
        return await jmisgoodevent.edit(
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await jmisgoodevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await jmisgoodevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
        ...
        ms = (end - start).seconds
        jmisgood = f"https://graph.org/{response['path']}"
        await jmisgoodevent.edit(
            f"**الرابط**:  [اضغط هنا]({jmisgood})\
