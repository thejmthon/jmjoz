...
async def participant_check(channel, user_id):
    try:
        await jmisgood(GetParticipantRequest(channel, int(user_id)))
        return True
...
@jmisgood.ar_cmd(pattern="اجباري ?(.*)")
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, jmisgood.uid):
            return await event.reply(
...
@jmisgood.ar_cmd(pattern="تعطيل الاجباري")
