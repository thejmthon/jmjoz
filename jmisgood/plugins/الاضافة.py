...
@jmisgood.ar_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    ...
    try:
        await jmisgood(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@jmisgood.ar_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@jmisgood.ar_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    legen_ = event.text[10:]
    jmisgood_chat = legen_.lower
    restricted = ["@super_jmthon", "@jmthon_support"]
    jmisgood = await edit_or_reply(event, f"**جارِ اضأفه الاعضاء من  ** {legen_}")
    if jmisgood_chat in restricted:
        return await jmisgood.edit(
            event, "**- لا يمكنك اخذ الاعضاء من مجموعه السورس العب بعيد ابني  :)**"
        )
    sender = await event.get_sender()
    me = await event.client.get_me()
    if not sender.id == me.id:
        await jmisgood.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    else:
        await jmisgood.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    if event.is_private:
        return await jmisgood.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await jmisgood.edit(
    ...
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await jmisgood(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await jmisgood.edit(
                f"**▾∮تتم الأضافة **\n\n• اضيف `{s}` \n•  خطأ بأضافة `{f}` \n\n**× اخر خطأ:** `{error}`"
            )
