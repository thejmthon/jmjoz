...
from jmisgood.sql_helper.MOrakb import *

from ..Config import Config

# from jmisgood.sql_helper.mo5zn import *


@jmisgood.ar_cmd(incoming=True, func=lambda e: e.is_group, edited=False, forword=None)
...
@jmisgood.ar_cmd(pattern=r"تخزين")
async def add_morakb_handler(event):
    msg = event.text.split()
    username = msg[1]
    user = await jmisgood.get_entity(username)
...
@jmisgood.ar_cmd(pattern=r"الغاء تخزين")
...
@jmisgood.ar_cmd(pattern=r"متابعة")
async def add_morakb_handler(event):
    msg = event.text.split()
    username = msg[1]
    user = await jmisgood.get_entity(username)
...
@jmisgood.ar_cmd(pattern=r"الغاء متابعة")
async def remove_morakb_handler(event):
    msg = event.text.split()
    username = msg[2]
    user = await jmisgood.get_entity(username)
