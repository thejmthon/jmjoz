...
async def ban_user(chat_id, i, rights):
    try:
        await jmisgood(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
    except Exception as exc:
        return False, str(exc)


@jmisgood.ar_cmd(pattern="بوتي$")
async def _(event):
    TG_BOT_USERNAME = Config.TG_BOT_USERNAME
    await event.reply(f"**❃ البوت الخاص بك هو** \n {TG_BOT_USERNAME}")


@jmisgood.ar_cmd(pattern="حالتي$")
async def _(event):
    text = "/start"
...
@jmisgood.on(events.NewMessage(outgoing=False, pattern="/roz"))
async def _(event):
...
@jmisgood.on(events.NewMessage(outgoing=False, pattern="^تحية"))
async def _(event):
    user = await event.get_sender()
    if user.id == 1280124974:
        await event.reply(
...
@jmisgood.ar_cmd(pattern="تفليش بالبوت$", groups_only=True)
async def banavot(event):
    ...
    try:
        await jmisgood(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in jmisgood.iter_participants(chat_id):
        ...
        await jmisgood.send_message(chat_id, usrtxt)
...
@jmisgood.ar_cmd(pattern="الغاء التفليش", groups_only=True)
