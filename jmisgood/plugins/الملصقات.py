...
async def delpack(jmthonevent, conv, args, packname):
    try:
        ...
        await jmisgood(unblock("stickers"))
        await conv.send_message("/delpack")
...
@jmisgood.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@jmisgood.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@jmisgood.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@jmisgood.ar_cmd(pattern="معلومات_ألملصق$")
