...
from . import edit_delete, jmisgood, logging
...
autopic_path = os.path.join(os.getcwd(), "jmisgood", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "jmisgood", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "jmisgood", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await jmisgood.upload_file(autophoto_path)
        try:
            if i > 0:
                await jmisgood(
                    functions.photos.DeletePhotosRequest(
                        await jmisgood.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await jmisgood(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await jmisgood(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await jmisgood(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@jmisgood.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@jmisgood.ar_cmd(pattern="اسم وقتي$")
async def _(event):
...
@jmisgood.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@jmisgood.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
...
jmisgood.loop.create_task(digitalpicloop())
jmisgood.loop.create_task(autoname_loop())
jmisgood.loop.create_task(autobio_loop())
