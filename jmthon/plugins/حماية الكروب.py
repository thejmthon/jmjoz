...
@jmthon.ar_cmd(pattern="قفل ?(.*)", groups_only=True, require_admin=True)
async def _(event):
...
@jmthon.ar_cmd(pattern="فتح ?(.*)", groups_only=True, require_admin=True)
async def _(event):
    input_str = event.pattern_match.group(1)
    chat_id = event.chat_id
    if not event.is_group:
...
@jmthon.ar_cmd(pattern="الاعدادات$", groups_only=True)
async def _(event):
    res = ""
...
@jmthon.ar_cmd(incoming=True, forword=None)
...
@jmthon.ar_cmd(pattern=f"البوتات ?(.*)")
