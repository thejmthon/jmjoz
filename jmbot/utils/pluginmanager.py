...
from jmbot import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"jmbot/plugins/{shortname}.py")
        checkplugins(path)
        name = f"jmbot.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"jmbot.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
    ...
    try:
        name = f"jmbot.plugins.{shortname}"
        for i in reversed(range(len(jmbot._event_builders))):
            ev, cb = jmbot._event_builders[i]
            if cb.__module__ == name:
                del jmbot._event_builders[i]
