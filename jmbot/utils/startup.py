...
async def saves():
    ...
    try:
        ...
        channel_usernames = ["jmthon", "RR7PP", "thejmthon", "KYOYK", "Quuran_karim", "jmthon_support"]
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"jmbot/{folder}/*.py"
        plugin_path = f"jmbot/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كروب بوت جمثون", jmbot, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    await load_plugins(folder="jmbot", extfolder=cfolder)
