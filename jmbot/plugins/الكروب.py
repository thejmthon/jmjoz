...
async def get_users(show):
    ...
    jmbotevent = await edit_or_reply(show, "**⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(jmbotevent, mentions)


@jmbot.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    jmbotevent = await edit_or_reply(event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**")
    chat = await get_chatinfo(event, jmbotevent)
    caption = await fetch_info(chat, event)
    try:
        await jmbotevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**⌔∮ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await jmbotevent.edit("**⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, jmbotevent):
    ...
    try:
        ...
        try:
            ...
            await jmbotevent.edit("**⌔∮ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
        except ChannelPrivateError:
            await jmbotevent.edit("**⌔∮ هذه مجموعة أو قناة خاصة أو لقد تمّ حظري منه ⛞**")
            return None
        except ChannelPublicGroupNaError:
            await jmbotevent.edit("**⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await jmbotevent.edit(str(err))
            return None
    return chat_info
