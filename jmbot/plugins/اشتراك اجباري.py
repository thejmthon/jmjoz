...
async def participant_check(channel, user_id):
    try:
        await jmbot(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, jmbot.uid):
            return await event.reply(
...
@jmbot.ar_cmd(pattern="تعطيل الاجباري")
