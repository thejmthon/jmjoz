...
@jmbot.ar_cmd(pattern="كروباتي$")
async def oeo(event):
    result = await jmbot(functions.channels.GetGroupsForDiscussionRequest())
    alist = []
    for item in result.chats:
        ...
        alist.append(roz)
    if alist:
        await jmbot.send_message("me", "\n".join(alist))


@jmbot.ar_cmd(pattern="الحاظرهم$")
async def main(event):
    result = await jmbot(functions.contacts.GetBlockedRequest(offset=0, limit=1000000))
    alist = []
...
@jmbot.ar_cmd(pattern="نوعه (.*)")
async def se(event):
    exe = event.text[5:]
    x = await jmbot.get_entity(exe)
...
@jmbot.ar_cmd(pattern="احذف (.*)")
async def se(event):
    exe = event.text[5:]
    await jmbot.get_dialogs()
    chat = exe
    await jmbot.delete_dialog(chat, revoke=True)
