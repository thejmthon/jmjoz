...
from . import jmbot, song_download
...
@jmbot.ar_cmd(pattern="بحث(320)?(?:\s|$)([\s\S]*)")
...
@jmbot.ar_cmd(pattern="فيديو(?:\s|$)([\s\S]*)")
...
@jmbot.ar_cmd(pattern="(ا(ل)?ا(س)?م)(?:\s|$)([\s\S]*)")
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await jmbot(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
