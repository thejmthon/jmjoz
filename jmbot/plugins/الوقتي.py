...
from . import edit_delete, jmbot, logging
...
autopic_path = os.path.join(os.getcwd(), "jmbot", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "jmbot", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "jmbot", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        file = await jmbot.upload_file(autophoto_path)
        try:
            if i > 0:
                await jmbot(
                    functions.photos.DeletePhotosRequest(
                        await jmbot.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await jmbot(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        LOGS.info(name)
        try:
            await jmbot(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    while AUTOBIOSTART := gvarstatus("autobio") == "true":
        ...
        LOGS.info(bio)
        try:
            await jmbot(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
@jmbot.ar_cmd(pattern="الصورة الوقتية$")
async def _(event):
    downloader = SmartDL(digitalpfp, digitalpic_path, progress_bar=False)
    downloader.start(blocking=False)
    while not downloader.isFinished():
...
@jmbot.ar_cmd(pattern="بايو وقتي$")
async def _(event):
...
@jmbot.ar_cmd(pattern="انهاء ([\s\S]*)")
async def _(event):
...
jmbot.loop.create_task(digitalpicloop())
jmbot.loop.create_task(autoname_loop())
jmbot.loop.create_task(autobio_loop())
