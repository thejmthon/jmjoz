...
from . import BOTLOG, BOTLOG_CHATID, jmbot
...
@jmbot.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    jmbotevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await jmbotevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await jmbotevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
        ...
        ms = (end - start).seconds
        jmbot = f"https://graph.org/{response['path']}"
        await jmbotevent.edit(
            f"**الرابط**:  [اضغط هنا]({jmbot})\
