...
from jmbot.sql_helper.MOrakb import *

from ..Config import Config

# from jmbot.sql_helper.mo5zn import *


@jmbot.ar_cmd(incoming=True, func=lambda e: e.is_group, edited=False, forword=None)
...
@jmbot.ar_cmd(pattern=r"تخزين")
async def add_morakb_handler(event):
    msg = event.text.split()
    username = msg[1]
    user = await jmbot.get_entity(username)
...
@jmbot.ar_cmd(pattern=r"الغاء تخزين")
...
@jmbot.ar_cmd(pattern=r"الغاء متابعة")
async def remove_morakb_handler(event):
    msg = event.text.split()
    username = msg[2]
    user = await jmbot.get_entity(username)
