...
@jmbot.ar_cmd(pattern="انضمام ([\s\S]*)")
async def lol(event):
    ...
    try:
        await jmbot(functions.channels.JoinChannelRequest(bol))
        await event.edit("**- تم الانضمام بنجاح  ✓**")
    except Exception as e:
        await event.edit(str(e))


@jmbot.ar_cmd(pattern="اضافه ([\s\S]*)")
async def _(event):
...
@jmbot.ar_cmd(pattern="ضيف ([\s\S]*)", groups_only=True)
async def get_users(event):
    ...
    if not sender.id == me.id:
        await jmbot.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    else:
        await jmbot.edit("**▾∮ تتم العملية انتظر قليلا ...**")
    if event.is_private:
        return await jmbot.edit("- لا يمكنك اضافه الاعضاء هنا")
    s = 0
    f = 0
    error = "None"
    await jmbot.edit("**▾∮ حالة الأضافة:**\n\n**▾∮ تتم جمع معلومات المستخدمين 🔄 ...⏣**")
    async for user in event.client.iter_participants(event.pattern_match.group(1)):
        try:
            ...
            tol = f"@{user.username}"
            lol = tol.split("`")
            await jmbot(InviteToChannelRequest(channel=event.chat_id, users=lol))
            s = s + 1
            await jmbot.edit(
                f"**▾∮تتم الأضافة **\n\n• اضيف `{s}` \n•  خطأ بأضافة `{f}` \n\n**× اخر خطأ:** `{error}`"
            )
