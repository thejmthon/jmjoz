...
@jmbot.ar_cmd(pattern="تحميل صوتي(?:\s|$)([\s\S]*)")
async def download_audio(event):
    ...
    for url in urls:
        try:
            ...
            vid_data = {"title": url, "uploader": "jmbot", "formats": []}
        startTime = time()
...
@jmbot.ar_cmd(pattern="تحميل فيديو(?:\s|$)([\s\S]*)")
...
@jmbot.ar_cmd(pattern="انستا(?: |$)([\s\S]*)")
    ...
    async with event.client.conversation(v1) as conv:
        ...
        async with event.client.conversation(v2) as conv:
            try:
                ...
                await jmbot(unblock("videomaniacbot"))
                v2_flag = await conv.send_message("/start")
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
...
@jmbot.ar_cmd(pattern="نتائج(?: |$)(\d*)? ?([\s\S]*)")
