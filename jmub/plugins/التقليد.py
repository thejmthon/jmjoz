...
@jmub.ar_cmd(pattern="الغاء تقليد$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@jmub.ar_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@jmub.ar_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@jmub.ar_cmd(incoming=True, edited=False)
