...
@jmub.ar_cmd(pattern="(لصورة|تحويل صورة)$")
async def _(event):
...
@jmub.ar_cmd(pattern="(لملصق|تحويل ملصق)$")
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
...
@jmub.ar_cmd(pattern="الملف لصورة$")
...
@jmub.ar_cmd(pattern="(تحويل متحركة|لمتحركة)$")
async def _(event):
...
@jmub.ar_cmd(pattern="تحويل الى (صوتي|بصمة)")
async def _(event):
