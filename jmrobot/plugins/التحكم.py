...
@jmrobot.ar_cmd(pattern="اوامر التحكم")
async def hi(event):
    await edit_or_reply(
        event,
...
@jmrobot.ar_cmd(pattern="التحكم (تفعيل|تعطيل)$")
...
@jmrobot.ar_cmd(pattern="اضف متحكم(?:\s|$)([\s\S]*)")
...
@jmrobot.ar_cmd(pattern="ازالة متحكم(?:\s|$)([\s\S]*)")
async def _(event):
    replied_user, error_i_a = await get_user_from_event(event)
    if replied_user is None:
        return
...
jmrobot.loop.create_task(_init())
