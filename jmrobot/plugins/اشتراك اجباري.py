...
async def participant_check(channel, user_id):
    try:
        await jmrobot(GetParticipantRequest(channel, int(user_id)))
        return True
...
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, jmrobot.uid):
            return await event.reply(
...
@jmrobot.ar_cmd(pattern="تعطيل الاجباري")
