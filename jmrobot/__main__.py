...
import jmrobot
from jmrobot import BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
try:
    ...
    jmrobot.loop.run_until_complete(setup_bot())
...
try:
    ...
    jmrobot.loop.create_task(saves())
...
try:
    ...
    jmrobot.loop.run_until_complete(mybot())
...
async def startup_process():
    if jmrobot.uid in blacklisted_users:
...
jmrobot.loop.run_until_complete(startup_process())

if len(sys.argv) in {1, 3, 4}:
    with contextlib.suppress(ConnectionError):
        jmrobot.run_until_disconnected()
else:
    jmrobot.disconnect()
