...
from jmrobot import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    ...
    """
    try:
        await jmrobot.connect()
        config = await jmrobot(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == jmrobot.session.server_address:
                if jmrobot.session.dc_id != option.id:
                    LOGS.warning(
                        f"اصلاح الداتا {jmrobot.session.dc_id}" f" الى {option.id}"
                    )
                jmrobot.session.set_dc(option.id, option.ip_address, option.port)
                jmrobot.session.save()
                break
        bot_details = await jmrobot.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await jmrobot.start(bot_token=Config.TG_BOT_USERNAME)
        jmrobot.me = await jmrobot.get_me()
        jmrobot.uid = jmrobot.tgbot.uid = utils.get_peer_id(jmrobot.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(jmrobot.me)
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await jmrobot(UnblockRequest("@R0R77"))
        await jmrobot(UnblockRequest("@jmthon_bot"))
        await jmrobot(
        ...
        await jmrobot.edit_folder("@jmthon_bot", folder=1)  # عمل ارشيف للبوت
        channel_usernames = [
            "jmthon",
            "RR7PP",
            "thejmthon",
        ...
        for channel_username in channel_usernames:
            try:
                channel = await jmrobot.get_entity(channel_username)
                await jmrobot(JoinChannelRequest(channel=channel))
            except Exception as e:
                LOGS.error(f"{e}")
...
async def mybot():
    jmrobot_USER = jmrobot.me.first_name
    The_razan = jmrobot.uid
    rz_ment = f"[{jmrobot_USER}](tg://user?id={The_razan})"
    f"ـ {rz_ment}"
    f"⪼ هذا هو بوت خاص بـ {rz_ment} يمكنك التواصل معه هنا"
    starkbot = await jmrobot.tgbot.get_me()
    perf = "[ جمثون ]"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await jmrobot.send_message("@jmthon_bot", "/start")
            ...
            await asyncio.sleep(1)
            await jmrobot.send_message("@BotFather", "/setinline")
            await asyncio.sleep(1)
            await jmrobot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await jmrobot.send_message("@BotFather", perf)
...
async def startupmessage():
    if not gvarstatus("DEPLOY"):
        ...
        try:
            if BOTLOG:
                await jmrobot.tgbot.send_message(
                    BOTLOG_CHATID,
    ...
    try:
        if msg_details:
            await jmrobot.check_testcases()
            message = await jmrobot.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**الان السورس شغال طبيعي.**"
            await jmrobot.edit_message(msg_details[0], msg_details[1], text)
...
@jmrobot.tgbot.on(CallbackQuery(data=re.compile(b"initft_(\\d+)")))
async def deploy(e):
...
@jmrobot.tgbot.on(CallbackQuery(data=re.compile(b"initbk_(\\d+)")))
async def ineiq(e):
    CURRENT = int(e.data_match.group(1))
    if CURRENT == 1:
        return await e.edit(
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"jmrobot/{folder}/*.py"
        plugin_path = f"jmrobot/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await jmrobot.get_entity(BOTLOG_CHATID)
        ...
        photobt = await jmrobot.upload_file(file="razan/pic/Jmthonp.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت جمثون", jmrobot, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await jmrobot.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
        ...
        photobt = await jmrobot.upload_file(file="razan/pic/Jmthonp.jpg")
        _, groupid = await create_supergroup(
            "مجموعة التخزين", jmrobot, Config.TG_BOT_USERNAME, descript, photobt
        )
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await jmrobot.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await jmrobot.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="jmrobot", extfolder=cfolder)
