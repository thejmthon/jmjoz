...
from jmrobot import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"jmrobot/plugins/{shortname}.py")
        checkplugins(path)
        name = f"jmrobot.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"jmrobot.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    jmrobot.remove_event_handler(i)
    ...
    try:
        name = f"jmrobot.plugins.{shortname}"
        for i in reversed(range(len(jmrobot._event_builders))):
            ev, cb = jmrobot._event_builders[i]
            if cb.__module__ == name:
                del jmrobot._event_builders[i]
