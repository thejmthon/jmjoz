...
from jmsource import CMD_HELP, LOAD_PLUG
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"jmsource/plugins/{shortname}.py")
        checkplugins(path)
        name = f"jmsource.plugins.{shortname}"
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules[f"jmsource.plugins.{shortname}"] = mod
        LOGS.info(f"تم تحميل ملف {shortname}")
...
def remove_plugin(shortname):
        ...
        for cmdname in cmd:
            if cmdname in LOADED_CMDS:
                for i in LOADED_CMDS[cmdname]:
                    jmsource.remove_event_handler(i)
    ...
    try:
        name = f"jmsource.plugins.{shortname}"
        for i in reversed(range(len(jmsource._event_builders))):
            ev, cb = jmsource._event_builders[i]
            if cb.__module__ == name:
                del jmsource._event_builders[i]
