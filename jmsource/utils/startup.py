...
from jmsource import BOTLOG, BOTLOG_CHATID, PM_LOGGER_GROUP_ID
...
async def setup_bot():
    ...
    """
    try:
        await jmsource.connect()
        config = await jmsource(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == jmsource.session.server_address:
                if jmsource.session.dc_id != option.id:
                    LOGS.warning(
                        f"اصلاح الداتا {jmsource.session.dc_id}" f" الى {option.id}"
                    )
                jmsource.session.set_dc(option.id, option.ip_address, option.port)
                jmsource.session.save()
                break
        bot_details = await jmsource.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await jmsource.start(bot_token=Config.TG_BOT_USERNAME)
        jmsource.me = await jmsource.get_me()
        jmsource.uid = jmsource.tgbot.uid = utils.get_peer_id(jmsource.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(jmsource.me)
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await jmsource(UnblockRequest("@R0R77"))
        await jmsource(UnblockRequest("@jmthon_bot"))
        await jmsource(
        ...
        await jmsource.edit_folder("@jmthon_bot", folder=1)  # عمل ارشيف للبوت
        channel_usernames = [
            "jmthon",
            "RR7PP",
            "thejmthon"
        ...
        for channel_username in channel_usernames:
            try:
                ...
                await jmsource(JoinChannelRequest(channel=channel))
            except Exception as e:
                LOGS.error(f"{e}")
...
async def mybot():
    jmsource_USER = jmsource.me.first_name
    The_razan = jmsource.uid
    rz_ment = f"[{jmsource_USER}](tg://user?id={The_razan})"
    f"ـ {rz_ment}"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            await jmsource.send_message("@jmthon_bot", "/start")
            ...
            await asyncio.sleep(1)
            await jmsource.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await jmsource.send_message("@BotFather", perf)
...
async def startupmessage():
    ...
    try:
        if msg_details:
            await jmsource.check_testcases()
            message = await jmsource.get_messages(msg_details[0], ids=msg_details[1])
            text = message.text + "\n\n**الان السورس شغال طبيعي.**"
            await jmsource.edit_message(msg_details[0], msg_details[1], text)
...
@jmsource.tgbot.on(CallbackQuery(data=re.compile(b"initft_(\\d+)")))
async def deploy(e):
...
@jmsource.tgbot.on(CallbackQuery(data=re.compile(b"initbk_(\\d+)")))
async def ineiq(e):
    CURRENT = int(e.data_match.group(1))
    if CURRENT == 1:
        return await e.edit(
...
async def load_plugins(folder, extfolder=None):
    ...
    if extfolder:
        ...
        path = f"jmsource/{folder}/*.py"
        plugin_path = f"jmsource/{folder}"
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await jmsource.get_entity(BOTLOG_CHATID)
        ...
        _, groupid = await create_supergroup(
            "كروب بوت جمثون", jmsource, Config.TG_BOT_USERNAME, descript, photobt
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        try:
            entity = await jmsource.get_entity(PM_LOGGER_GROUP_ID)
            if not isinstance(entity, types.User) and not entity.creator:
                if entity.default_banned_rights.send_messages:
...
async def install_externalrepo(repo, branch, cfolder):
    ...
    if response.code != 200:
        LOGS.error(errtext)
        return await jmsource.tgbot.send_message(BOTLOG_CHATID, errtext)
    await runcmd(gcmd)
    if not os.path.exists(cfolder):
        LOGS.error(
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا "
        )
        return await jmsource.tgbot.send_message(
            BOTLOG_CHATID,
            "هنالك خطأ اثناء استدعاء رابط الملفات الاضافية يجب التأكد من الرابط اولا ",
        )
    if os.path.exists(rpath):
        await runcmd(f"pip3 install --no-cache-dir -r {rpath}")
    await load_plugins(folder="jmsource", extfolder=cfolder)
