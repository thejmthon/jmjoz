...
@jmsource.ar_cmd(pattern="الحاسبة(?:\s|$)([\s\S]*)")
async def icalc(e):
    if e.client._bot:
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @jmsource  ~ @RR7PP
@jmsource.tgbot.on(CallbackQuery(data=re.compile(b"calc(.*)")))
@check_owner
...
# 𝗧𝗲𝗹𝗲𝗚𝗿𝗮𝗠 : @jmsource  ~ @RR7PP
@jmsource.tgbot.on(CallbackQuery(data=re.compile(b"recalc")))
@check_owner
async def _(e):
    m = [
        "AC",
