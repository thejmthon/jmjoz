...
@jmsource.ar_cmd(pattern="تفعيل الترجمة")
...
@jmsource.ar_cmd(pattern="تعطيل الترجمة")
...
@jmsource.on(events.NewMessage(outgoing=True))
    ...
    if gvarstatus("translateen"):
        ...
        try:
            ...
            if translation.text != event.message.message:
                await jmsource.edit_message(event.message, translation.text)
        except:
            pass
