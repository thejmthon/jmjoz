...
from jmsource import Convert, jmsource
...
@jmsource.ar_cmd(pattern="دائري ?((-)?s)?$")
...
@jmsource.ar_cmd(pattern="(لصورة|تحويل صورة)$")
async def _(event):
...
@jmsource.ar_cmd(pattern="(لملصق|تحويل ملصق)$")
async def _(event):
    reply_to_id = await reply_id(event)
    reply = await event.get_reply_message()
    if not reply:
...
@jmsource.ar_cmd(pattern="لملف ([\s\S]*)")
async def get(event):
    name = event.text[5:]
    if name is None:
...
@jmsource.ar_cmd(pattern="لكتابة$")
async def get(event):
    reply = await event.get_reply_message()
    mediatype = await media_type(reply)
...
@jmsource.ar_cmd(pattern="الملف لصورة$")
...
@jmsource.ar_cmd(pattern="(تحويل متحركة|لمتحركة)$")
async def _(event):
...
@jmsource.ar_cmd(pattern="تحويل الى (صوتي|بصمة)")
async def _(event):
