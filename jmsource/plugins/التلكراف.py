...
from . import BOTLOG, BOTLOG_CHATID, jmsource
...
@jmsource.ar_cmd(pattern="(ت(ل)?ك(راف)?) ?(m|t|ميديا|نص)(?:\s|$)([\s\S]*)")
async def _(event):
    jmsourceevent = await edit_or_reply(event, "** ⌔∮ جار انشاء رابط تلكراف**")
    if BOTLOG:
    ...
    if not event.reply_to_msg_id:
        return await jmsourceevent.edit(
    ...
    if input_str in ["ميديا", "m"]:
        ...
        await jmsourceevent.edit(f"**⌔∮ تم التحميل الى {downloaded_file_name}**")
        if downloaded_file_name.endswith((".webp")):
            resize_image(downloaded_file_name)
        try:
            media_urls = upload_file(downloaded_file_name)
        except exceptions.TelegraphException as exc:
            await jmsourceevent.edit(f"** ⌔∮ خطأ : **\n**{exc}**")
        ...
        ms = (end - start).seconds
        jmsource = f"https://graph.org/{response['path']}"
        await jmsourceevent.edit(
            f"**الرابط**:  [اضغط هنا]({jmsource})\
