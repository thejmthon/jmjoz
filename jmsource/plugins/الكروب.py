...
@jmsource.ar_cmd(pattern="المشرفين(?: |$)(.*)")
async def _(event):
...
@jmsource.ar_cmd(pattern="البوتات(?: |$)(.*)")
async def _(event):
...
@jmsource.ar_cmd(pattern="الاعضاء(?: |$)(.*)")
async def get_users(show):
    ...
    jmsourceevent = await edit_or_reply(show, "**⌔∮ جاري سحب قائمة معرّفات الأعضاء 🝛**")
    try:
        ...
        mentions += " " + str(e) + "\n"
    await edit_or_reply(jmsourceevent, mentions)


@jmsource.ar_cmd(pattern="معلومات(?: |$)(.*)")
async def info(event):
    jmsourceevent = await edit_or_reply(event, "**⪼ يتمّ جلب معلومات الدردشة، إنتظر ⅏**")
    chat = await get_chatinfo(event, jmsourceevent)
    caption = await fetch_info(chat, event)
    try:
        await jmsourceevent.edit(caption, parse_mode="html")
    except Exception as e:
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID, f"**⌔∮ هناك خطأ في معلومات الدردشة ✕ : **\n`{str(e)}`"
            )
        await jmsourceevent.edit("**⌔∮ حدث خطأ ما، يرجى التحقق من الأمر ⎌**")


async def get_chatinfo(event, jmsourceevent):
    ...
    try:
        ...
        try:
            ...
            await jmsourceevent.edit("**⌔∮ لم يتمّ العثور على القناة/المجموعة ✕**")
            return None
        except ChannelPrivateError:
            await jmsourceevent.edit(
                "**⌔∮ هذه مجموعة أو قناة خاصة أو لقد تمّ حظري منه ⛞**"
            )
            return None
        except ChannelPublicGroupNaError:
            await jmsourceevent.edit("**⌔∮ القناة أو المجموعة الخارقة غير موجودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            await jmsourceevent.edit(str(err))
            return None
    return chat_info
