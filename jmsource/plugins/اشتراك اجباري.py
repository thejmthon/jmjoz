...
async def participant_check(channel, user_id):
    try:
        await jmsource(GetParticipantRequest(channel, int(user_id)))
        return True
...
@jmsource.ar_cmd(pattern="اجباري ?(.*)")
async def fsub(event):
    ...
    if not str(channel).startswith("@"):
        ...
        if not await participant_check(channel, jmsource.uid):
            return await event.reply(
...
@jmsource.ar_cmd(pattern="تعطيل الاجباري")
