...
async def delpack(jmthonevent, conv, args, packname):
    try:
        ...
        await jmsource(unblock("stickers"))
        await conv.send_message("/delpack")
...
@jmsource.ar_cmd(pattern="ملصق(?:\s|$)([\s\S]*)")
async def kang(args):
    photo = None
    emojibypass = False
    is_anim = False
...
@jmsource.ar_cmd(pattern="حزمة(?:\s|$)([\s\S]*)")
...
@jmsource.ar_cmd(pattern="تحويل بملصق(?:\s|$)([\s\S]*)")
...
@jmsource.ar_cmd(pattern="معلومات_ألملصق$")
